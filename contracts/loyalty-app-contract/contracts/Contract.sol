...
contract Platform is ERC2771Recipient, Token, Users, Offers {
    ...
    function createUser(string memory name, string memory profileURL) public returns (bool) {
        _createUser(_msgSender(), name, profileURL, 'customer');
        return true;
    }
    ...
    constructor(
        ...
        owner = _msgSender();
        _createUser(_msgSender(), name, profileURL, 'admin');
    ...
    function updateRoleToPartner(address user) public override onlyOwner returns (bool) {
        ...
        userMappings[user].role = "partner";
        return true;
    }

    function demoteRoleToCustomer(address user) public override onlyOwner returns (bool) {
        if(user == owner) revert PlatformError("Cannot change the owner's role");
        if(userMappings[user].registered != true) revert PlatformError("Not a registered user");

        userMappings[user].role = "customer";
        return true;
    }
    ...
    function rewardForPurchase(address customer, uint256 purchaseAmount) public onlyOwner returns (bool) {
        ...
        transferFrom(owner, customer, userMappings[owner].name, userMappings[customer].name, tokenAmount, "Reward For Purchase");
    ...
    function rewardCustomer(address customer, uint256 tokenAmount) public {
        ...
        transferFrom(owner, _msgSender(), userMappings[owner].name, userMappings[_msgSender()].name, tokenAmount, "Buy Tokens");
        transferFrom(_msgSender(), customer, userMappings[_msgSender()].name, userMappings[customer].name, tokenAmount, "Reward Tokens");
    }
    ...
    function buyOffer(uint256 _id) public returns (bool) {
        ...
        transferFrom(_msgSender(), receiver, userMappings[_msgSender()].name, userMappings[receiver].name, currentOffer.priceInTokens, transactionType);
